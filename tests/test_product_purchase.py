...
from utils.csv_data_provider import get_random_valid_user

def test_search_and_add_product_to_cart(page):
    """Search for product, add to cart, and verify in cart."""
...
def test_add_random_product_and_logout(page):
    """End-to-End Test with Random User Login & 3-Level Product Randomization."""
    home_page = HomePage(page)
    login_page = LoginPage(page)
    products_page = ProductsPage(page)
    cart_page = CartPage(page)

    # 1. Select a random valid user from CSV & Login
    random_user = get_random_valid_user()
    print(f"\n[Random User]: {random_user['username']}")
    ...
    print(f"[Level 1 - Random Category]: {category_name}")
